"""ModelSlim MXFP4 W4A8 offline scheme for MoE layers on Ascend NPU (SRT).

Loads weights pre-quantised by msmodelslim: packed-fp4 uint8 weights + uint8
block scales (block_size=32, E8M0 +127 biased). The layout transform and the
forward pass are delegated to ``NPUMXFP4W4A8MoEMethod`` -- the same kernel the
online W4A8 MXFP MoE path uses.

Mirrors vllm-ascend ``AscendW4A8MXFPDynamicFusedMoEMethod`` exactly.
"""
...
from sglang.srt.hardware_backend.npu.quantization.moe_methods import NPUMXFP4W4A8MoEMethod
...
# Block (group) size of the msmodelslim W4A8_MXFP export format.
MXFP4_BLOCK_SIZE = 32
# Two fp4 values packed into one uint8 → K dimension is halved.
MXFP4_PACK_FACTOR = 2


class ModelSlimMXFP4W4A8MoEScheme(ModelSlimMoEScheme):
    """
    Offline MXFP4 W4A8 MoE scheme that creates weights for either the
    ...
    ``NPUMXFP4W4A8MoEMethod.process_weights_after_loading`` to take its offline
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        # Mirrors vllm-ascend AscendW4A8MXFPDynamicFusedMoEMethod.get_weight /
