...
# ---------------------------------------------------------------------------
class NPUMXFP4W4A8FusedMoEMethod(FusedMoEMethodBase):
    """Online MXFP4 W4A8 FusedMoE entry point (``--quantization mxfp_w4a8`` on A5).

    Owns the BF16 weight placeholders and the runner wiring; the per-gmm work
    lives in ``NPUMXFP4W4A8MoEMethod``, which quantises the weights to packed
    MXFP4 at load time. The offline ModelSlim path reuses that same kernel via
    ``ModelSlimMXFP4W4A8MoEScheme`` instead of this class.
    ...
    def __init__(self, quant_config: Optional["QuantizationConfig"] = None):
        super().__init__()
        self.quant_config = quant_config
        self.w13_kernel = NPUMXFP4W4A8MoEMethod()
        self.w2_kernel = NPUMXFP4W4A8MoEMethod()
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        # Weights load in params_dtype (BF16/FP16) and are quantised to packed
        # MXFP4 in process_weights_after_loading.
        w13_weight = torch.nn.Parameter(
            torch.empty(
                num_experts,
                2 * intermediate_size_per_partition,
